...
static __global__ void flash_attn_vec_ext_f16(
    ...
    if (use_logit_softcap && !(D == 128 || D == 256)) {
        ...
        GGML_UNUSED(ne32); GGML_UNUSED(ne33);
        GGML_UNUSED(nb31); GGML_UNUSED(nb32); GGML_UNUSED(nb33);
        GGML_UNUSED(nb01); GGML_UNUSED(nb02);
...
#pragma unroll
        for (int j0 = 0; j0 < ncols; j0 += nwarps) {
            ...
            if (ncols > 2 && ic0 + j >= (int32_t)ne01.z) {
#pragma unroll
...
#pragma unroll
            for (int i0 = 0; i0 < D/sizeof(int); i0 += WARP_SIZE) {
                quantize_q8_1_to_shared<half2, WARP_SIZE>(Q_f + 4*i0, scale, tmp_q_i32 + i0, tmp_q_ds + i0/QI8_1);
            }
        }
...
#pragma unroll
            for (int i0 = 0; i0 < D/2; i0 += WARP_SIZE) {
                ...
                const float2 tmp = ncols <= 2 || ic0 + j < (int32_t)ne01.z ? Q_f2_j[i] : make_float2(0.0f, 0.0f);
...
#pragma unroll
    for (int j_VKQ = 0; j_VKQ < ncols; ++j_VKQ) {
        if (ncols > 2 && ic0 + j_VKQ >= (int32_t)ne01.z) {
            break;
        }
    ...
    if (gridDim.y != 1 && tid < ncols && (ncols <= 2 || ic0 + tid < (int32_t)ne01.z)) {

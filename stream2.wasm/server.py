"""
hyni voice scheduler - backend

Serves the static WASM frontend and proxies LLM calls so that provider
API keys never reach the browser.

Env vars (see .env.example):
    LLM_PROVIDER      openai | anthropic          (default: openai)
    LLM_API_KEY       provider API key            (required for /api/chat)
    LLM_MODEL         override default model      (optional)
    GOOGLE_CLIENT_ID  OAuth2 public client id     (optional; empty disables cal)
    LLM_TIMEOUT_S     upstream timeout seconds    (default: 30)
"""
...
# ---------------------------------------------------------------------------
# System prompt (moved verbatim from index.html, ASCII-only)
# ---------------------------------------------------------------------------
...
async def coop_coep_headers(request, call_next):
    """COOP/COEP on every response so the WASM pthread pool gets SharedArrayBuffer.
    Using 'credentialless' (not 'require-corp') so Google GSI and fonts
    load without per-resource crossorigin surgery."""
...
async def chat(req: ChatRequest):
    ...
    spec = PROVIDERS[provider](messages, system, _env("LLM_MODEL") or None)
    ...
    # Guard: 'done' is only valid when both name and dateTime slots are filled.
    # If the LLM prematurely transitions to done (e.g. via prompt injection or
    # hallucination), reject it so the frontend never attempts event creation
